...
func (gq *GroupQuery) QueryHerbaria() *HerbariumQuery {
...
func (gq *GroupQuery) WithHerbaria(opts ...func(*HerbariumQuery)) *GroupQuery {
	query := (&HerbariumClient{config: gq.config}).Query()
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withHerbaria; query != nil {
		if err := gq.loadHerbaria(ctx, query, nodes,
			func(n *Group) { n.Edges.Herbaria = []*Herbarium{} },
			func(n *Group, e *Herbarium) { n.Edges.Herbaria = append(n.Edges.Herbaria, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range gq.withNamedHerbaria {
		if err := gq.loadHerbaria(ctx, query, nodes,
			func(n *Group) { n.appendNamedHerbaria(name) },
			func(n *Group, e *Herbarium) { n.appendNamedHerbaria(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GroupQuery) loadHerbaria(ctx context.Context, query *HerbariumQuery, nodes []*Group, init func(*Group), assign func(*Group, *Herbarium)) error {
	...
	query.Where(predicate.Herbarium(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.HerbariaColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_herbaria
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_herbaria" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_herbaria" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (gq *GroupQuery) WithNamedHerbaria(name string, opts ...func(*HerbariumQuery)) *GroupQuery {
	query := (&HerbariumClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
