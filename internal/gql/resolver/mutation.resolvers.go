...
// DeletePeriod is the resolver for the deletePeriod field.
func (r *mutationResolver) DeletePeriod(ctx context.Context, id int) (string, error) {
	client := ent.FromContext(ctx)
	err := client.Period.DeleteOneID(id).Exec(ctx)
	if err != nil {
		return "", err
	}
